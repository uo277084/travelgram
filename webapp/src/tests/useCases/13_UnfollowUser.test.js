...
describe('Dejar de seguir tests', () => {
    test('el perfil que se visualiza es del usuario en sesión', async () => {
        ...
        waitFor(() => {
            expect(screen.getByText('Dejar de seguir')).toBeNull();
        }, { timeout: 3000 });
    });

    test('el usuario en sesión no sigue al usuario y aparece el botón de dejar de seguir', async () => {
        ...
        userService.checkFollow.mockResolvedValueOnce(true);
        publicationService.getPublicationsByUser.mockResolvedValueOnce({ publications: [] });
